import { FaBars, FaBell, FaSearch, FaMicrophone } from "react-icons/fa";
...
useEffect(() => {
  const timer = setTimeout(() => {
    ...
    if (searchChache[searchText]) {
      ...
      setsuggestions(searchChache[searchText]);
    } else {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white flex items-center justify-between px-5 py-3 shadow-sm">
      ...
      {/* Right */}
      <div className="flex items-center gap-5">
        <button className="px-4 py-2 bg-gray-100 rounded-full">+ Create</button>
